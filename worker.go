...
func (w *Worker) WorkOne() (didWork bool) {
	...
	// worker func onstart
	w_ok := func(j *Job) error { return nil }

	wf_os, ok := w.m["onStart"]
	if !ok {
		wf_os = w_ok
	}

	wf_oe, ok := w.m["onError"]
	if !ok {
		wf_oe = w_ok
	}

	wf_of, ok := w.m["onFinish"]
	if !ok {
		wf_of = w_ok
	}

	if err = wf_os(j); err != nil {
		log.Printf("attempting to run on start job %d: %v", j.ID, err)
	}

	if err = wf(j); err != nil {
		if j.ErrorCount > int32(w.MaxRetries) {

			if err = wf_of(j); err != nil {
				log.Printf("attempting to run on finish job %d: %v", j.ID, err)
			}
			...
			if err = wf_oe(j); err != nil {
				log.Printf("attempting to run on error job %d: %v", j.ID, err)
			}
		}
		return
	}
...
func NewWorkerPool(c *Client, wm WorkMap, count int, maxRetries int) *WorkerPool {
	return &WorkerPool{
		c:          c,
		WorkMap:    wm,
		Interval:   defaultWakeInterval,
...
func (w *WorkerPool) Start() {
	...
	for i := range w.workers {
		...
		w.workers[i].MaxRetries = w.MaxRetries
		go w.workers[i].Work()
	}
}
